...
func construirEntornoLogro_Usuario(t *testing.T) http.Handler {
	t.Helper()

	luFake := nuevoLogro_UsuarioFake()
	luService := rlc.NewLogro_UsuarioService(luFake)

	srv := handlers.NewServer(handlers.Deps{Logro_Usuario: luService})
...
func TestCrearLogro_Usuario_Exitoso(t *testing.T) {
	h := construirEntornoLogro_Usuario(t)
